...
import { AddPartnerTagCommand } from "./add-partner-tag.command";
import { Result } from "@is-kontrol/shared-result";

// Etiket ekleme idempotenttir: var olan etiket tekrar eklenmez.
export class AddPartnerTagHandler extends PartnerCommandHandlerBase {
  constructor(
  ...
  async execute(command: AddPartnerTagCommand): Promise<Result<void, PartnerError>> {
    return this.mutatePartner(command.partnerId, async (partner) => {
      partner.addTag(command.tag);
