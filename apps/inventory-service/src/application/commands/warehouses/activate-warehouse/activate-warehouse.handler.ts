...
import { ActivateWarehouseCommand } from "./activate-warehouse.command";
import { Result } from "@is-kontrol/shared-result";

export class ActivateWarehouseHandler extends WarehouseCommandHandlerBase {
  constructor(
  ...
  async execute(
    command: ActivateWarehouseCommand,
  ): Promise<Result<void, WarehouseError>> {
    return this.change(command.id, "warehouse.activated", (warehouse) =>
      warehouse.activate(),
    );
  }
}
