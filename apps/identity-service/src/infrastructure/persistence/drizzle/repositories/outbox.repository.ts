...
/* Outbox'a yazan transaction commit olduğunda publisher worker'ı uyandıran
 * Postgres NOTIFY kanalı. NOTIFY transaction içinde gönderilir ve Postgres
 * bunu ancak commit'te iletir; rollback'te hiç iletilmez. */
export const OUTBOX_WAKEUP_CHANNEL = 'outbox_event_added';
...
export class DrizzleOutboxRepository implements IOutboxRepository {
  ...
  async save(event: Omit<OutboxEvent, 'id' | 'createdAt' | 'publishedAt'>): Promise<void> {
    ...
    // Aynı transaction'daki eş bildirimleri Postgres tek NOTIFY'a indirger.
    await this.db.execute(sql`select pg_notify(${OUTBOX_WAKEUP_CHANNEL}, '')`);
  }
